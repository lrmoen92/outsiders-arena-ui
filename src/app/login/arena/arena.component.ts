...
import { Battle, Character, Player} from 'src/app/model/api-models';
...
findBattle() {
	let playerName = this.opponentName;
	if (playerName) {
...
connectByPlayerName(name : string) {
	...
	console.log(name);
	this.httpClient.get(URLS.playerArena + this.playerId + '/' + name).subscribe(
	  x => {
